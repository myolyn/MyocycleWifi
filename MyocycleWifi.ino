...
wl_status_t status;
char password[PASS_LENGHT] = {0x00};
char ssid[SSID_LENGHT] = {0x00};
char current_ssid[SSID_LENGHT] = {0x00};

//Contact info

char contactName[NAME_LENGTH] = {0x00};
char contactEmail[EMAIL_LENGTH] = {0x00};
char contactPhone[PHONE_LENGTH] = {0x00};
char UserID[7] = {0x00,0x00,0x00,0x00,0x00,0x00,0x00};
char User_Pin[5] = {0x00,0x00,0x00,0x00};
char UserID_default[7] = {0x31,0x32,0x33,0x34,0x35,0x36,0x37};
char User_Pin_default[5] = {0x39,0x38,0x37,0x36};
...
unsigned char Wifi_end_bytes_A[4] = {0xAA, 0x44, 0xA4, 0x4A};
unsigned char Wifi_handshake_A[2] = {0xBB, WIFI_HANDSHAKE};
unsigned char Wifi_status_A[3] = {0xBB, WIFI_STATUS, WIFI_DISCONNECTED};
unsigned char Wifi_scan_A[24] = {0xBB, WIFI_SCAN};
unsigned char Wifi_checkUpdate_A[3] = {0xBB, WIFI_CHECK_UPDATE, 0x00};
unsigned char Wifi_versionInfo_A[25] = {0xBB, WIFI_VERSIONREQUEST};
unsigned char Wifi_deviceIPInfo_A[7] = {0xBB, WIFI_DEVICEIP};
unsigned char Wifi_deviceMACInfo_A[20] = {0xBB, WIFI_DEVICEMAC};
unsigned char Wifi_deviceSSID_A[32] = {0xBB, WIFI_CURRENT_SSID};
unsigned char Wifi_userProfileNumber_A[3] = {0xBB, WIFI_USER_NUMBER_CHECK};

unsigned char Wifi_userProfileParameters_A[9] = {0xBB, WIFI_USER_PARAMETER_REQUEST, 0x01, 0x00, 0x00, 0x6A, 0xAF, 0x8C, 0x06};
unsigned char Wifi_requestProfile_A[3] = {0xBB, WIFI_SCAN_NEXT, 0x00};
unsigned char user_profile_status[50] = {0x00};
unsigned char Wifi_workoutSummary_A[2] = {0xBB, WIFI_SUMMARY};
unsigned char Wifi_updateSurveryCounter[4] = {0xBB, WIFI_UPDATE_SURVEY_COUNTER, 0x01, 0x06};
unsigned char Wifi_clearFRAM[2] = {0xBB,WIFI_CLEAR_FRAM};
unsigned char Wifi_contactInfoReceived[2] = {0xBB, WIFI_CONTACT_INFO};
unsigned char Wifi_proVersionStatus[3] = {0xBB, WIFI_IS_PRO_VERSION};
unsigned char Wifi_serverLoginStatus[3] = {0xBB, WIFI_LOGIN,0x00};
unsigned char Wifi_serverLoginCheck[3] = {0xBB, WIFI_LOGIN_CHECK,0x00};
...
bool connect()
{
  ...
  else
  {
    ...
    for (index8 = 0; index8 < SSID_LENGHT; index8++)
    {
      fram.write8(FRAM1, 0x32 + index8, ssid[index8]);
    }
    fram.write8(FRAM1, 0x52, PSWDsize);
    for (index8 = 0; index8 < PASS_LENGHT; index8++)
    {
...
bool uploadSummary(void)
{
  ...
  int httpPort = 80;
  if (!client.connect(host, httpPort)) {
   // Serial.println("Upload_Summary Connection Failed");
    return false;
  }
...
bool uploadContactInfo(void)
{
  ...
  int httpPort = 80;
  if (!client.connect(host, httpPort)) {
   // Serial.println("Upload Contact Info failed");
    return false;
  }
...
bool startWorkout(void)
{
  ...
  int httpPort = 80;
  if (!client.connect(host, httpPort)) {
    //Serial.println("Opening Workout Session Connection Failed");
    return false;
  }
...
bool uploadWorkout(void)
{
  ...
  int httpPort = 80;
  if (!client.connect(host, httpPort)) {
    //Serial.println("uploadWorkout Connection Failed");
...
bool KernelCode()
{
  ...
  int httpPort = 80;
  if (!client.connect(host, httpPort)) {
    //Serial.println("Kernel Connection Failed");
   // delay(100);
    return false;
  }
...
bool AppCode()
{
  ...
  int httpPort = 80;
  if (!client.connect(host, httpPort)) {
    //Serial.println("AppCode Connection Failed");
   // delay(100);
    return false;
  }
    ...
    if (receiveBufferIndex >= 4 && (receiveBuffer[receiveBufferIndex - 3]) == 0xAA && (receiveBuffer[receiveBufferIndex - 2]) == 0x44  && (receiveBuffer[receiveBufferIndex - 1]) == 0xA4 && (receiveBuffer[receiveBufferIndex]) == 0x4A)
    {
...

      {
        ...
        case WIFI_CONNECT:
          ...
          PSWDsize = PASS_LENGHT;
          strncpy(ssid, WiFi.SSID(receiveBuffer[2] - 1), SSIDsize);
          for (index8 = 0; index8 < (SSID_LENGHT - SSIDsize); index8++)
          {
        ...
        //  Serial.println(PSWDsize);
          ...
          for (index8 = 0; index8 < (PASS_LENGHT - PSWDsize); index8++) {
